...
void joint_planning_trapezoidal::init(double qi[], double qf[], double max_vel[], double max_acc[], int num_joint) {
    ...
    for(int i=0;i<num_joint;i++){
        ...
        if ( (pow(max_vel[i], 2.0) / max_acc[i]) >= (qf[i]-qi[i]) ){
            ...
            tc[i] = pow(fabs((qf[i] - qi[i])) / max_acc[i], 0.5);
            tf[i] = 2*tc[i];
        }
        else{
            triangolare[i] = false;
            ...
            tf[i] = fabs((qf[i] - qi[i] - max_acc[i] * pow(tc[i], 2.0)) / max_vel[i] + 2 * tc[i]);
        }
        //ROS_INFO("TF = %f", tf[i]);
    }

    //ROS_INFO("INIZIALIZZAZIONE: qi: %f  %f  %f", this->qi[0],this->qi[1],this->qi[2]);
    //ROS_INFO("INIZIALIZZAZIONE: qf: %f  %f  %f", this->qf[0],this->qf[1],this->qf[2]);
}

void joint_planning_trapezoidal::plan(double time){
    for (int i=0; i<num_joint; i++){
        if (triangolare[i] && qf[i]>=qi[i]){
            if (time <= tc[i]){
        ...
        else if (triangolare[i]==0 && qf[i]>=qi[i]) {
            if (time <= tc[i]){
                q[i] = qi[i] + 0.5 * max_acc[i] * pow(time, 2.0);
                v[i] = max_acc[i] * time;
                a[i] = max_acc[i];
